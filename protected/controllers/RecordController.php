...
//			),
	...
	public function actionPreop()
	{
            ...
            $curl=TRUE;
            /************** 
             * some heloer functions
             */
            // turns dashes to Null values when they come back from Lenstar
            ...
            //print_r($xml);
            //$json_string = json_encode($xml);
            Yii::trace('Entered lenStarPreop.php Vers2.1 20140515');
            //echo "\nEntered lenStarPreop.php Vers2.1 20131107";
            //$result_array = json_decode($json_string, TRUE);
            ...
            $Sex = strtoupper(substr($Sex, 0, 1));
            $Thisuser = (string) $xml->Login['User'];
            $Thispwd = (string) $xml->Login['Pwd'];  // in this version it will be a guid
            $Technician=(string) $xml->Login['Technician'];
            $Office=(string) $xml->Login['Office'];
            $ChartID = (string) $xml->Patient['ID'];
            $Surgeon = (string) $xml->Login['Surgeon'];
